...
from flask_openapi import swag_from, Swagger
...
swagger_description = "docs/test_refs_basePath_API.yaml"
...
def get_cost():
    result = dict(description="The best place", cost=dict(currency="EUR", value=123456))
...
def test_swag(client, specs_data):
    ...
    with client.get("apispec_1.json") as response:
        ...
        assert "/api/get_costs" not in paths, "/api/get_cost NOK: %r" % response_data

    req_data = dict(
        level=2,
        location=dict(
            name="my preferred location",
            position=dict(latitude=47.352735, longitude=0.593682),
        ),
    )
